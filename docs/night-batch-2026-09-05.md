...
## 第四波（02:00-02:05，训练模式 + 结算升级）
- **训练模式**：单人世界进入即训练（服务端跳过自建城、玩家自动攻方、刷 6v8 双方 AI、5s 自动开局、回合循环复用）；主菜单右侧新「SOLO TRAINING」卡点击进选世界（顶栏单机入口原有）；dedicated 服不受影响
- **结算卡升级**：状态帧新增 lastResult 字段；ROUND OVER 显示胜方大字（进攻方黄/防守方蓝）+ 340ms 淡入上浮入场动画
- dev：`dev-5ca5aaa`（含 4ca 全部：TAB/大厅/兵种/死亡部署/HitMarker/测试/训练/结算；已热更本地服+sync）
